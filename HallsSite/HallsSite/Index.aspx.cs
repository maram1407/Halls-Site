...
namespace HallsSite
{
    public partial class Index : System.Web.UI.Page
    {
        ...
        /*
        protected void PriceEnd_TextChanged(object sender, EventArgs e)
        {
            if (PriceEnd.Text != string.Empty)
            {
                end = Convert.ToInt32(PriceEnd.Text);
                SqlConnection con = new SqlConnection(CS);
                con.Open();
                string qr = "select A.*,B.*,A.HPrice,B.Name as ImageName from tblHalls A   " +
                    ...
                    "where  A.HPrice <= " + PriceEnd.Text + " order by A.HID desc";
                SqlDataAdapter da = new SqlDataAdapter(qr, con);
                string text = ((TextBox)sender).Text;
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        */
        /*
        protected void PriceStart_TextChanged(object sender, EventArgs e)
        {
            if (PriceStart.Text != string.Empty)
            {
                start = Convert.ToInt32(PriceStart.Text);
                SqlConnection con = new SqlConnection(CS);
                con.Open();
                string qr = "select A.*,B.*,A.HPrice,B.Name as ImageName from tblHalls A   " +
                    "cross apply( select top 1 * from tblHallImages B where B.HID= A.HID order by B.HID desc )B " +
                    "where  A.HPrice >= " + PriceStart.Text + " order by A.HID desc";
                SqlDataAdapter da = new SqlDataAdapter(qr, con);
                string text = ((TextBox)sender).Text;
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        */
        ...
        protected void Count_TextChanged(object sender, EventArgs e)
        {
            if (Count.Text != string.Empty)
            {
                ...
                string qr = "select A.*,B.*,A.HPrice,B.Name as ImageName from tblHalls A   " +
                    "cross apply( select top 1 * from tblHallImages B where B.HID= A.HID order by B.HID desc )B " +
                    "where  A.Number >= " + Count.Text + " order by A.HID desc";
                SqlDataAdapter da = new SqlDataAdapter(qr, con);
                string text = ((TextBox)sender).Text;
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        */
        ...
        protected void ddlType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (ddlType.SelectedIndex!=0)
            {
                ...
                string qr = "select A.*,B.*,A.HPrice,B.Name as ImageName from tblHalls A   " +
                    "cross apply( select top 1 * from tblHallImages B where B.HID= A.HID order by B.HID desc )B " +
                    "where  A.HCityID = " + ddlType.SelectedItem.Value + " order by A.HID desc";
                SqlDataAdapter da = new SqlDataAdapter(qr, con);
                //string text = ((TextBox)sender).Text;
                DataSet ds = new DataSet();
                da.Fill(ds);
        ...
        {
            if ((ddlType.SelectedIndex != 0)&&(PriceEnd.Text != string.Empty)&&
                (Count.Text != string.Empty) &&(PriceStart.Text != string.Empty))
            {
                SqlConnection con = new SqlConnection(CS);
                con.Open();
                string qr = "select A.*,B.*,A.HPrice,B.Name as ImageName from tblHalls A   " +
